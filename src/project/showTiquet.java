...
public class showTiquet extends JPanel {
    ...
    public void pagarTiquet() {
    	String updTiq = "UPDATE REBUT SET PAGAT = 'S' WHERE NUMC = ?"; // Corregido el uso del signo de interrogación
        try (PreparedStatement statement = con.prepareStatement(updTiq)) {
            
            statement.setInt(1, numC); // Establecer el parámetro en la consulta
            
            int rowsAffected = statement.executeUpdate(); // Usar executeUpdate() para UPDATE
            
            if (rowsAffected > 0) {
                JOptionPane.showMessageDialog(null, "Ticket actualizado con éxito");
            } else {
